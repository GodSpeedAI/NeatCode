...
function readSafe(root, path, maxBytes = 512 * 1024) {
  try {
...
export function localImports(root, path, { limit = 25 } = {}) {
  const text = readSafe(root, path);
  if (!text) return [];
...
export function relatedTests(root, path, { limit = 10 } = {}) {
  const stem = basename(path, extname(path));
  const out = [];
  for (const candidate of trackedFiles(root)) {
...
export function expandContext(root, changedPaths, { limit = 40 } = {}) {
  ...
  for (const path of changedPaths.slice(0, limit)) {
    ...
    rings.push({
      ...
      imports: localImports(root, path),
      callers: likelyCallers(root, path),
      tests: relatedTests(root, path),
    });
  }
  return rings;
}
