// MECHANICAL PORT of vercel-labs/skills src/agents.ts — do not hand-edit.
// Regenerate with /tmp/port-agents.mjs after an upstream refresh, then re-check
// the deviations listed in lib/env/upstream/UPSTREAM.md.
// Changes vs upstream: node: imports; xdg-basedir replaced by XDG_CONFIG_HOME
// env-or-default; TypeScript types stripped; registry loads lazily via
// loadUpstreamAgents(ctx) so tests can inject a fake home/cwd.
...
export function loadUpstreamAgents(ctx = {}) {

const home = ctx.home ?? homedir();
// Use xdg-basedir (not env-paths) to match OpenCode/Amp/Goose behavior on all platforms.
const configHome = xdgConfig ?? join(ctx.home ?? home, '.config');
...
function isMiniMaxCodeInstalled(
  homeDir = home,
  pathExists = existsSync
) {
...
function isPositAssistantInstalled(
  homeDir = home,
  pathExists = existsSync
) {
...
const agents = {
  ...
  astrbot: {
    ...
    detectInstalled: async () => {
      return existsSync(join((ctx.cwd ?? process.cwd()), 'data/skills')) || existsSync(join(home, '.astrbot'));
    },
  },
  'autohand-code': {
    name: 'autohand-code',
  ...
  codebuddy: {
    ...
    detectInstalled: async () => {
      return existsSync(join((ctx.cwd ?? process.cwd()), '.codebuddy')) || existsSync(join(home, '.codebuddy'));
    },
  },
  codemaker: {
    name: 'codemaker',
  ...
  continue: {
    ...
    detectInstalled: async () => {
      return existsSync(join((ctx.cwd ?? process.cwd()), '.continue')) || existsSync(join(home, '.continue'));
    },
  },
  cortex: {
    name: 'cortex',
  ...
  jazz: {
    ...
    detectInstalled: async () => {
      return existsSync(join(home, '.jazz')) || existsSync(join((ctx.cwd ?? process.cwd()), '.jazz'));
    },
  },
  junie: {
    name: 'junie',
  ...
  replit: {
    ...
    detectInstalled: async () => {
      return existsSync(join((ctx.cwd ?? process.cwd()), '.replit'));
    },
  },
  reasonix: {
    name: 'reasonix',
  ...
  promptscript: {
    ...
    detectInstalled: async () => {
      return (
        existsSync(join((ctx.cwd ?? process.cwd()), '.promptscript')) ||
        existsSync(join((ctx.cwd ?? process.cwd()), 'promptscript.yaml'))
      );
    },
  },
  adal: {
