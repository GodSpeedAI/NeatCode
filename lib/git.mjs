...
export function acquireDiff(source, { cwd = process.cwd() } = {}) {
  const common = ['--no-color', '--find-renames', '--no-ext-diff', '--src-prefix=a/', '--dst-prefix=b/'];
...
/** Files that git considers dirty right now — evidence of an unclean tree. */export function workingTreeStatus(cwd) {
 ...
 * Used for baseline guard scans (what did this file look like before?).
 */
export function showFile(rev, path, cwd) {
  const { stdout, status } = git(['show', `${rev}:${path}`], { cwd, allowFail: true });
